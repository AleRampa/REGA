...
% isobars (bar)
stepSize = 0.5;
isobars1 = 0.001:0.001*stepSize:0.01;
isobars2 = 0.02:0.01*stepSize:0.1;
isobars3 = 0.2:0.1*stepSize:10;
isobars4 = 11:1*stepSize:100;
isobars5 = 105:5*stepSize:250;
isobars6 = 260:10*stepSize:290;
isobars7 = 300:100*stepSize:700;

iso1 = [isobars1 isobars2 isobars3 isobars4 isobars5 isobars6 isobars7];
iso2 = [isobars1 isobars2 isobars3 isobars4 isobars5];

built_in_species =  ["H2O" "N2" "H2" "CO" "CO2" "N2O" "CH4O" "CH4" "C2H6"...
...
elseif (ischar(varargin{1}) && ismember(varargin{1},built_in_species))
    % Stopper in case of you missed something
    sure = input("ARE YOU SURE TO GO ON DOWNLOADING " + varargin{1} + " DATA ?? \n",'s');
    if ~strcmpi(sure,'yes')
        return
    end
%               H2O        %N2        %H2         CO        CO2       N2O
    idcas_list = ["C7732185" "C7727379" "C1333740" "C630080" "C124389" "C10024972"...
        ...%CH4O     CH4    C2H6     C2H4     C3H8      C3H6     C3H4
        "C67561" "C74828" "C74840" "C74851" "C74986" "C115071" "C74997"...
        ...%C4H10   C5H12     C6H14     C6H12     C6H6       NH3
        "C106978" "C109660" "C110543" "C110827" "C71432"  "C7664417"...
     ...
     %calculating every MM using element MMs
    %        H2O         N2      H2         CO       CO2           N2O
    MM_list = [H_MM*2+O_MM, N_MM*2, H_MM*2, C_MM+O_MM, C_MM+O_MM*2, N_MM*2+O_MM, ...
        ...%  CH4O              CH4          C2H6           C2H4
        C_MM+H_MM*4 + O_MM,  C_MM+H_MM*4, C_MM*2+H_MM*6, C_MM*2+H_MM*4,...
        ...%  C3H8          C3H6         C3H4           C4H10
        C_MM*3+H_MM*8, C_MM*3+H_MM*6, C_MM*3+H_MM*4, C_MM*4+H_MM*10,...
        ...
        N_MM+H_MM*3, He_MM, O_MM*2, C_MM*2+H_MM*2+F_MM*4  H_MM*2];
          ...
          %H2O   %N2   %H2   CO    CO2    N2O  CH4O   CH4   C2H6   C2H4
    Ref_list ={Ref3  Ref4  Ref1  Ref1  Ref2   Ref1  Ref1   Ref1  Ref1   Ref1 ...
        ...%  C3H8   C3H6    C3H4   C4H10  C5H12  C6H14  C6H12  C6H6
              Ref2   Ref2    Ref2    Ref2   Ref1   Ref1  Ref1   Ref1  ...
        ...%  NH3     He     O2   R134a(C2H2F4)  pH2 (para-H2)
              Ref5   Ref1   Ref6      Ref2        Ref1 };
               ...
               %H2O   %N2   %H2    CO    CO2   N2O  CH4O   CH4   C2H6  C2H4
    isobars_list = { iso1, iso1, iso1, iso1, iso1, iso2, iso1, iso1, iso1, iso1, ...
      ...%  C3H8   C3H6    C3H4   C4H10  C5H12  C6H14  C6H12  C6H6
           iso1,   iso1,   iso2,  iso2,  iso2,   iso2,  iso2, iso2,  ...
      ...%  NH3     He     O2   R134a(C2H2F4)  pH2 (para-H2) 
           iso1,   iso1,  iso1,     iso1,        iso1 }; 
    species = varargin(1);
    idcas = idcas_list(built_in_species == varargin{1});
    MM = MM_list(built_in_species == varargin{1});
    Ref = Ref_list(built_in_species == varargin{1});
    isobars = isobars_list(built_in_species == varargin{1});
elseif (ischar(varargin{1}) && strcmpi(varargin{1},'reduced'))
    ...
    idcas = { 'C7732185' 'C7727379' 'C1333740' 'C124389' ...
         ... CH4      He          O2      R134a(C2H2F4)        N2O 
           'C74828' 'C7440597' 'C7782447'  'C811972'      'C10024972'}; ...
